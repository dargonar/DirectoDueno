...
import sys
import pickle
from cache_compress import to_binary, from_binary
...
class GeoModel(db.Model):
  ...
  def update_location(self):
    ...
    if self.location:
      ...
      self.location_geocells = sorted(tmp+primos)
  ...
  def bounding_box_fetch(query, bbox, max_results=1000
    ...
    the_box,primo = geocell.best_bbox_search_cells(bbox, cost_function)
    
    current_cursor = None
    
    if the_box:
      query.filter('location_geocells =', the_box)
      
      if cursor_key is not None:
        logging.info('bounding_box_fetch() using cursor:'+cursor_key)
        query.with_cursor(cursor_key)
      else:
        logging.info('bounding_box_fetch() NOT using cursor:')
      ...
      for entity in query:
        ...
        datu = str(entity.name()).split(',')
        lat = float(datu[1])
        lon = float(datu[2])
        if (lat >= bbox.south and
            lat <= bbox.north and
            lon >= bbox.west and
            lon <= bbox.east):
          #results.append(db.Key.from_path('Property', datu[0]))
          results.append(db.Key(datu[0]))
            
    m_the_box = the_box if the_box is not None else ''
    return results, m_the_box + '|-|' + str(primo) + '-' + str(bbox), current_cursor
  
  def passy(self, passy_param):
    return True
  ...
  @staticmethod
  def bounding_box_fetch_count(query, bbox, cost_function=None):
    ...
    i = 0
    for p in query_geocells:
      logging.info('geomodel::bounding_box_fetch_count() query_geocells #%s "%s"' % (str(i),p))
      i+=1
    
    if query_geocells:
      #return 1
      return query.filter('location_geocells IN', query_geocells).count(limit=1000)
    return 0   
    
  @staticmethod
  def bounding_box_fetch_count2(query):
    logging.info('geomodel::bounding_box_fetch_count() query:"%s"' % str(query))
