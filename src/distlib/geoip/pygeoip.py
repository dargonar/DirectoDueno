...
Modified by Pau Sanchez (http://www.codigomanso.com/en)
'''
...
def disk_lookup (ip):
  '''
  Helper function that will perform a disk lookup to find given ip
  '''
  db = Database(os.path.join (os.path.dirname(__file__), 'GeoIP.dat'), False)
  info = db.lookup(ip)
  db.close()
